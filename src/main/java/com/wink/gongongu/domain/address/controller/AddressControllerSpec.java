...
public interface AddressControllerSpec {
    ...
    @Operation(
        ...
        description = """
            ...
            이미 기본 주소록으로 등록된 주소록이 존재시 그 주소록의 기본 주소록 설정이 해제됩니다.<br>
            기본 주소록으로 변경 시 유저의 리전이 변경됩니다. (도로명주소에서 앞 두음절 따오는 방식)
            """
    )
    ...
    @Operation(
        summary = "주소록 수정 API",
        description = """
            request에 담긴 정보가 통째로 변경됩니다. 따라서 변경이 안된 필드도 유지하여 그대로 넘겨줘야합니다.<br>
            기본 주소록으로 변경 시 유저의 리전이 변경됩니다. (도로명주소에서 앞 두음절 따오는 방식)<br>
            기본 주소록 설정을 해제하면 "기본 주소록은 무조건 하나 이상 존재해야합니다."라는 예외가 발생합니다.<br>
            """
    )
    UserAddressUpdateResponse updateAddress(UserPrincipal principal, Long addressId, UserAddressUpdateRequest request);
    ...
    void deleteAddress(UserPrincipal principal, Long addressId);
